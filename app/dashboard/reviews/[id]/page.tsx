...
import { getSnapshotSteps } from "@/lib/portal/shared";
...
export default async function ReviewDetailPage({
  ...
  const { data: instance } = await supabase
    ...
    .select("id, client_name, client_email, status, access_token, template_id, template_snapshot, ai_summary")
    .eq("id", id)
    .single();
  ...
  const { data: liveSteps } = snapshotSteps || instance.template_snapshot ? { data: null } : await supabase
    .from("template_steps")
