...
describe("checkAllRequiredStepsComplete", () => {
  ...
  it("uses the instance snapshot when the live template has changed", async () => {
    const supabase = createMockSupabase({
      template_steps: [{ id: "new-live-step" }],
      client_submissions: [{ step_id: "step-from-snapshot" }],
    });
    ...
    const result = await checkAllRequiredStepsComplete(
      ...
      "instance-1",
      "template-1",
      { steps: [{ id: "step-from-snapshot", step_order: 0, title: "Snapshot step", type: "short_text", is_required: true }] }
    );
