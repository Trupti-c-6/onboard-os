...
import { stepRejectedEmail, onboardingCompletedEmail } from "@/lib/email/templates";
import { checkAllRequiredStepsApproved } from "@/lib/portal/shared";
...
export async function approveSubmission(
  ...
  const { data: instance } = await supabase
    ...
    .select("id, status, template_id, template_snapshot, client_email, organization_id")
    .eq("id", instanceId)
    .single();

  if (instance) {
    const allApproved = await checkAllRequiredStepsApproved(
      supabase,
      instance.id,
    ...
    if (allApproved) {
      ...
      const portalUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/portal/${instanceToken}`;
      const { subject, html } = onboardingCompletedEmail(org?.name ?? "your provider", portalUrl);
      try {
        await sendEmail(instance.client_email, subject, html);
      } catch {
        // best-effort — the completion status is already saved regardless
      }
    } else if (instance.status === "submitted") {
      // First approval action on a fresh submission — mark it as actively
      // being worked, distinct from "submitted and untouched".
      await supabase.from("client_instances").update({ status: "in_review" }).eq("id", instance.id);
    }
  }
