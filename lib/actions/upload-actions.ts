...
export async function getSignedUploadUrl(input: unknown): Promise<SignedUrlResult> {
  ...
  const { data: instance } = await supabase
    ...
    .select("id, organization_id, template_id, token_expires_at, template_snapshot")
  ...
  const { data: liveStep } = snapshotSteps || instance.template_snapshot ? { data: null } : await supabase
    .from("template_steps").select("id, type, is_required, validation_rules")
    .eq("id", parsed.data.stepId).eq("template_id", instance.template_id).single();
  const step = snapshotSteps?.find((candidate) => candidate.id === parsed.data.stepId) ?? liveStep;
  if (!step || step.type !== "file_upload") return { success: false, error: "Invalid onboarding step." };
...
export async function registerSubmissionAsset(input: unknown): Promise<RegisterAssetResult> {
  ...
  const { data: instance } = await supabase
    .from("client_instances")
    .select("id, organization_id, template_id, status, token_expires_at, template_snapshot")
    .eq("access_token", parsed.data.token)
    .single();

  if (!instance) return { success: false, error: "Invalid link." };
  if (new Date(instance.token_expires_at) < new Date()) return { success: false, error: "This link has expired." };
  ...
  const step = snapshotSteps?.find((candidate) => candidate.id === parsed.data.stepId) ?? liveStep;
  if (!step || step.type !== "file_upload") return { success: false, error: "Invalid onboarding step." };
  const rules = (step.validation_rules ?? {}) as { accepted_types?: string[]; max_size_bytes?: number };
  if (rules.max_size_bytes && parsed.data.fileSizeBytes > rules.max_size_bytes) return { success: false, error: "File is too large." };
  if (rules.accepted_types?.length) {
    const name = parsed.data.fileName.toLowerCase();
    const type = parsed.data.mimeType.toLowerCase();
    const accepted = rules.accepted_types.some((value) => value.startsWith(".") ? name.endsWith(value) : value.endsWith("/*") ? type.startsWith(value.slice(0, -1)) : type === value);
    if (!accepted) return { success: false, error: "File type is not accepted." };
  }
  ...
  const allComplete = await checkAllRequiredStepsComplete(supabase, instance.id, instance.template_id, instance.template_snapshot);
